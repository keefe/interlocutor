...
public class SimpleConversation implements Conversation<SimpleCriteria> {
  ...
  public List<Conversation<SimpleCriteria>> filter(SimpleCriteria criteria) {
    ...
    // TODO figure out criteria pattern, maybe subclasses of SimpleCriteria?
    if (criteria.getThreadId() != null) {
      SimpleConversation thread = new SimpleConversation(name + "#" + criteria.getThreadId());
      for (Message m : content()) {
        if (criteria.getThreadId().equals(m.getThreadId())) {
          thread.add(m);
        }
      }
    ...
    for (Message m : content()) {
      if (m.getThreadId() == null) {
        if (latest.latest() != null
            && (m.getTimestampSeconds() - latest.latest().getTimestampSeconds()) > criteria
                .getContinuitySeconds()) {
