...
public class ComprehendAdvisor<C> implements Advisor<C> {
  ...
  public ComprehendAdvisor() {
    ...
    comprehendClient = AmazonComprehendClientBuilder.standard().withCredentials(awsCreds).build();
  ...
  public SentimentAdvice detectSentiment(Conversation<C> conversation) {
    ...
    System.out.println("Total chars " + length + " total units " + (length / 300.0) + " cost "
        + 0.0001 * Math.max(length / 300.0, 3.0));
    DetectSentimentRequest detectSentimentRequest =
        new DetectSentimentRequest().withText(text.toString()).withLanguageCode("en");
  ...
  public KeyphraseAdvice detectKeyphrases(Conversation<C> conversation) {
    ...
    DetectKeyPhrasesRequest detectKeyPhrasesRequest =
        new DetectKeyPhrasesRequest().withText(text.toString()).withLanguageCode("en");
